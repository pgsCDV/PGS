...
public class MazeGame : MonoBehaviour {
    ...
    void GenerateAndSpawnAll() {
        print(manager.seed);
        GenerateAndSpawnMaze(manager.seed, pos1, new Vector2Int(8, 8));
    }
    ...
    void SpawnMaze(Transform parent, Vector2Int size) {
        ...
        for (int row = 0; row < size.x; row++) {
            for (int col = 0; col < size.y; col++) {
                ...
                if (cell.WallRight && col < size.y - 1) {
                ...
                if (cell.WallFront && row < size.x - 1) {
                    GameObject w = Instantiate(Wall, parent);
        ...
        if (Pillar != null) {
            for (int row = 0; row <= size.x; row++) {
                for (int col = 0; col <= size.y; col++) {
                    if (row == 0 || row == size.x || col == 0 || col == size.y) continue;
